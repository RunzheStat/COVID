...
getValueforThisLambda <-function(vlam, XR_t0_lag, A_t0_1_lag, popu, 
  ...
  init_belief_state = generateBeliefState(theta, XR_t0_lag, c(A_t0_1_lag, rep(0,T_t0 - 1))[1:(1+(lag - 1))], popu, lag = lag)
  ...
  one_traj <- function(m){
    ...
    for (tt in 1: (T_t0 - 1)) {
      ...
      if (tt %% decision.t == 1) { # decision point
          belife_state = generateBeliefState(theta, states[3, tt:(tt+lag)], As[tt:(tt+(lag - 1))], popu, lag = lag)
