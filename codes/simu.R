    ...
    updateCity_i <- function(city_i, is.A.point = T){
      ...
      if (is.A.point) {
        if (method == "ours") {
          ...
          init_state = generateBeliefState(hat.theta.t0, pre_XRs = city_i$state[3, (t0 - lag):t0]
                                           , pre_As = city_i$action[(t0 - lag):(t0-1)]
                                           , popu = city_i$popu
                                           , stochastic = stochastic)
          init_state = round(init_state)
          Xl = city_i$gdp
          decisions_k = rep(NA, 3)
          if (t0 == T_begin) {
            for (kk in 1:3){
              gsir = GSIR(city_i$state
                          , city_i$action
                          , hat.theta.t0, w, T_end, Xl, t0)
              agent = Agent(gsir, timesteps_per_episode = T_end, batch_size = 32)
              A = agent$train(100) + 1
              ...
              decisions_k[kk] = A
              if (kk == 2 & A == decisions_k[1]){
                break
              }
            }
            A = getmode(decisions_k)
          }else{
            gsir = GSIR(city_i$state
                        , city_i$action
                        , hat.theta.t0, w, T_end, Xl, t0)
            agent = Agent(gsir, timesteps_per_episode = T_end, batch_size = 32)
            for (kk in 1:3){
              agent$q_network = city_i$agent
              A = agent$train(100, e_tol = 5) + 1
              if(A >= 100){
                A = A - 100
                decisions_k[kk] = A
                if (A != 1)break
          ...
          city_i$agent = agent$q_network
          cat(t0, city_i$state[, t0], decisions_k, "  ", A, "\n")
          # cat(city_i$state[, t0], A, "\n")
        }
      }else{
