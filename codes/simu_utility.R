...
getmode <- function(v) {
  ...
  r = uniqv[which.max(tabulate(match(v, uniqv)))]
  if (is.na(r)) {
    return(v[1])
  }else{
    return(r)
...
plotParetoFrontierforSimu <-function(ours, DQN, oracle, point_size = 1, line_size = 1 
                                     , add_sd
                                     , sd_ours
                                     , sd_thre_XN
                                     , sd_thre_T3
                                     ...
                                     , r_thre_T3
                                     , r_thre_T2
                                     , RC_fixed, behav, city_name, origin_zero = F, inlog = F, echo = F, adjust_10 = T){
  ...
  policies = c("Pareto (threshold)", "mitigation", "suppresion", "behaviour", "threshold", "oracle", "Pareto (DQN)")
  
  colors <- c( 
    # "Pareto" = cbp1[4], "DQN" = cbp1[5], 
    "Pareto (threshold)" = cbp1[4], "Pareto (DQN)" = "#C3D7A4", #"#D16103", 
      # brewer.pal(n = 3, name = "RdBu")[3],
    "oracle" = cbp1[8], "threshold" = cbp1[2], "RC_fixed"=cbp1[3],
    "mitigation" = cbp1[6], "suppresion" = cbp1[1], "behaviour" = cbp1[7])
  shapes <- c(#"Pareto" = 16, 
    #"DQN" = 17, 
    "Pareto (threshold)" = 16, "Pareto (DQN)" = 17, 
    "oracle" = 1
  ...
  if(add_sd){
    if(adjust_10) sd_ours[2,] = sd_ours[2,] * 10
    gg = gg + geom_rect(data=dat, aes(xmin=R-sd_ours[1,]*1.96, xmax=R+sd_ours[1,]*1.96, ymin=C-sd_ours[2,]*1.96, 
  ...
  if (!missing(DQN)) {
    R_DQN = DQN[1,]
    # if (inlog)R_DQN = log10(R_DQN)
    C_DQN = DQN[2,]
    if(adjust_10) C_DQN = C_DQN * 10
    
    dat_DQN = data.frame(R = R_DQN, C = C_DQN)
    #dat_DQN$policy = "DQN"
    dat_DQN$policy = "Pareto (DQN)"
    
    dat_DQN$sdk_x = sd_DQN[1,]*1.96;
    dat_DQN$sdk_y = sd_DQN[2,]*1.96;
    gg =   gg + geom_point(data = dat_DQN, aes(x = R, y = C, color= policy, shape=policy), size=figure_size) + 
      geom_line(data = dat_DQN, aes(x = R, y = C, color= policy, shape=policy)
                , alpha = 1, show.legend = FALSE
                , size = line_size) 
    if(add_sd){
      if(adjust_10) dat_DQN$sdk_y = dat_DQN$sdk_y * 10
      gg = gg + geom_rect(data = dat_DQN, aes(xmin=R-sdk_x, xmax=R+sdk_x, ymin=C-sdk_y,
  ...
  if (!missing(r_thre_T2)) {
    ...
    gg =   gg + geom_point(data = dat_thre_T2, aes(x = R, y = C, color= policy, shape=policy), size=figure_size) + 
      geom_line(data = dat_thre_T2, aes(x = R, y = C, color= policy, shape=policy)
                ...
                , size = line_size) 
    if(add_sd){
      if(adjust_10) dat_thre_T2$sdk_y = dat_thre_T2$sdk_y * 10
  ...
  if (!missing(r_thre_T3)) {
    ...
    gg = gg + 
      ...
      geom_line(data = dat_thre_T3, aes(x = R, y = C, color= policy, shape=policy)
                ...
                , size = line_size)
    if(add_sd){
      if(adjust_10) sd_thre_T3[2,] = sd_thre_T3[2,] * 10
  ...
  if (!missing(behav)) {
    ...
    if(adjust_10) dat_behav$C = dat_behav$C * 10
  ...
  if (!missing(r_thre_XN)) {
    ...
    gg = gg + 
      ...
      geom_line(data = dat_thre_XN, aes(x = R, y = C, color = policy, shape = policy)
                ...
                , size = line_size)
    if (add_sd){
      if(adjust_10) sd_thre_XN[2,] = sd_thre_XN[2,] * 10
  ...
  ##########################################################################
  if(echo){print(gg)}
  xs = c(R, R_thre_XN, R_thre_T3, R_thre_T2)
  ys = c(C, C_thre_XN, C_thre_T3, C_thre_T2)
  if (!missing(behav)) {
    xs = c(xs, -behav[1])
    ys = c(ys, dat_behav$C)
  }
