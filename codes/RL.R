...
policySearch <- function(theta = NA, t0 = 12, T_end = 120,
  ...
  if (is.state.oracle) {
    ...
    belief_state_t0 = generateBeliefState(theta, XR_t0_lag, A_t0_1_lag, popu, lag = lag) # S, I, R
  }
    ...
    gridSearch <-function(lower_para1, upper_para1, step_para1, upper_para2, step_para2, lower_para2, tol = early_stop_tol){#lower_para2
      ...
      for (vlam1 in seq(lower_para1, upper_para1, step_para1)){
        ...
        for (vlam2 in seq(max(lower_para2,vlam1), upper_para2, step_para2)) {
          vlam = c(vlam1, vlam2)
          r  = getValueforThisLambda(vlam = vlam, XR_t0_lag = XR_t0_lag, A_t0_1_lag = A_t0_1_lag
                                     , popu = popu, t0 = t0, T_end = T_end
                                     , theta = theta, GDP_daily = GDP_daily, w = w, M = M, lag = lag
                                     , init_seed = s_seed, decision.t = decision.t
                                     , MC = MC, Y_smallest = value_best[1]); s_seed = s_seed + 1
          value = r[1:3]
          YRC_std = r[4:6]
  ...
  if (only_action) {
    ...
    return(list(lam = vlam_best, A = A, values = value_best))
  }
}
