...
finalPlot <- function(t0 = 12,
    ...
    plot_one_city <- function(l) {
      ...
      if (add.action) {
        ...
        for (t in 2:length(action)){
          if(action[t] != action[t - 1]){
            date = plots_i$data$date[t + 9] + 24 * 3600
            plots_i = plots_i + geom_text(x = date
                                          , y= 10
                                          , label = action[t])
          }
    ...
    ggs = mclapply(c(1:6), plot_one_city)
...
ggsave(
  filename = paste(out_path, "re_Figure2_lower_with_A.png", sep = ""),
  plot = gg_T,
  width = 8,
  height = 5,
  units = c("in"),
